...
insn_tests_hexagon = {
    "J2_jump": "{(riV); riV = (riV & ~(4 - 1)); JUMP((PC)+riV);}",
    "J2_jumpr": "{JUMP(RsV);}",
    "J2_jumpt": "{; if (((PuV) & 1)) { (riV);riV = (riV & ~(4 - 1)); JUMP((PC)+riV);; }}",
    "J2_jumpf": "{; if ((!((PuV) & 1))) { (riV);riV = (riV & ~(4 - 1)); JUMP((PC)+riV);; }}",
    "J2_jumprt": "{; if (((PuV) & 1)) { JUMP(RsV);; }}",
    "J2_jumprf": "{; if ((!((PuV) & 1))) { JUMP(RsV);; }}",
    "J4_cmpgti_tp0_jump_t": "{__COMPOUND_PART1__{ fWRITE_P0((((RsV>UiV)) ? 0xff : 0x00)); }__COMPOUND_PART1__ if ((P0_NEW & 1)) {(riV); riV = (riV & ~(4 - 1)); JUMP((PC)+riV);}}",
    "J2_call": "{(riV); riV = (riV & ~(4 - 1)); fCALL((PC)+riV); }",
    "J2_loop0r": "{ (riV); riV = (riV & ~(4 - 1)); fWRITE_LOOP_REGS0( (PC)+riV, RsV); HEX_REG_ALIAS_USR_NEW = ((REGFIELD(HEX_RF_WIDTH, HEX_REG_FIELD_USR_LPCFG)) ? deposit64(HEX_REG_ALIAS_USR_NEW, (REGFIELD(HEX_RF_OFFSET, HEX_REG_FIELD_USR_LPCFG)), (REGFIELD(HEX_RF_WIDTH, HEX_REG_FIELD_USR_LPCFG)), (((0)))) : HEX_REG_ALIAS_USR_NEW); }",
    ...
    "L4_return": "{ size8u_t tmp;  EA = (RsV); ; tmp = (size8u_t)(mem_load_u64(EA)); RddV = ((tmp) ^ (((uint64_t)((env->gpr[HEX_REG_ALIAS_FRAMEKEY]))) << 32)); fWRITE_SP(EA+8); JUMP(((int64_t)((int32_t)((RddV >> ((1) * 32)) & 0x0ffffffffLL))));}",
    ...
    "SL2_loadrd_sp": "{        EA = (env->gpr[HEX_REG_ALIAS_SP]) + uiV;    ; RddV = (size8u_t)(mem_load_u64(EA));}",
    "SL2_jumpr31_t": "{; if ((((env->pred[0])) & 1)) {JUMP((env->gpr[HEX_REG_ALIAS_LR]));}}",
    "SS2_storeh_io": "{        EA = RsV + uiV;    ; mem_store_u16(EA, ((int16_t)((RtV >> ((0) * 16)) & 0xffff)));}",
    "SS2_stored_sp": "{        EA = (env->gpr[HEX_REG_ALIAS_SP]) + siV;    ; mem_store_u64(EA, RttV);}",
    "SS2_storew_sp": "{        EA = (env->gpr[HEX_REG_ALIAS_SP]) + uiV;    ; mem_store_u32(EA, RtV);}",
    "SS2_allocframe": "{         EA = (env->gpr[HEX_REG_ALIAS_SP]) + -8;    ; mem_store_u64(EA, (((((uint64_t)((env->gpr[HEX_REG_ALIAS_LR]))) << 32) | ((uint32_t)((env->gpr[HEX_REG_ALIAS_FP])))) ^ (((uint64_t)((env->gpr[HEX_REG_ALIAS_FRAMEKEY]))) << 32))); fWRITE_FP(EA); ; fWRITE_SP(EA-uiV); }",
}
